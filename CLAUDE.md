...
- **Git** : ✅ dépôt privé `origin` = `https://github.com/ousseynou-faye/Plateforme_Biblio_FayeMath.git`, branche `main`. **Poussé jusqu'à l'étape 12** : `origin/main` = `c930bff`, en synchro avec le local. Commits de l'étape 12 : `0efd82d` (enums+entités), `d41752a` (base Drift), `c930bff` (modèles+erreurs), au-dessus des 2 commits de l'étape 11 (`51735e8`, `5d3894e`). **Note de fonctionnement** : le poste d'Ousseynou pousse automatiquement après commit (VS Code *Sync* ou équivalent) — les commits arrivent sur `origin/main` sans `git push` explicite de Claude Code. Toujours vérifier l'état réel avec `git status -sb` (et `git ls-remote origin` au besoin) plutôt que de supposer.
