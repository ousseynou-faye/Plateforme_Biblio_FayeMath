...
**Étape 16 — Détail d'un chapitre** (maquette V2.1, **écran 6**) : au clic sur un chapitre de la liste (étape 15), l'élève voit les **documents** du chapitre (les 8 types du kit, `docs/GLOSSAIRE.md`), **leur taille et leur statut gratuit/premium** (Feuille de Route, doc 04 : « les documents, leur taille, leur statut gratuit ou premium »). Elle dépend de l'étape 12 (entité `Ressource` faite) **ET** de l'étape 15 (liste faite : la ligne de chapitre y attend déjà une navigation, aujourd'hui un placeholder SnackBar). Ouvrir la maquette **écran 6** + `SPECIFICATIONS_V2_Plateforme.md` **avant de coder**. Points d'attention connus : (a) **aucune ressource n'existe encore en base** (le contenu réel relève de l'étape 18) — même situation qu'à l'étape 15, prévoir l'état « documents indisponibles » ; (b) le badge Gratuit/Premium réutilise `badge_premium_widget` (étape 11) et lit `ressource.premium` **tel quel** (dérivé côté serveur, jamais recalculé) ; (c) le **téléchargement réel** (dio + Storage) et l'ouverture du PDF relèvent des étapes 17 (lecteur) et de la Phase 3 — à cette étape on **affiche**, on ne télécharge pas encore.

> **Séquencement (Feuille de Route, doc 04).** **12 → 13 → 14 → 15 (faites) → 16 (détail d'un chapitre, écran 6, attend 12 ET 15)**. La prochaine étape est donc la **16**.

Ensuite les écrans restants de la V2.1 un par un (17 lecteur de documents, 18 mise en ligne du contenu…), en ouvrant à chaque fois la maquette (`FayeMath_Maquettes_Ecrans_V2.html`) et ses spécifications avant de coder.
