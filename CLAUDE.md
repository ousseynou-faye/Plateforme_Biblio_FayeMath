...
**Étape 14 — Écran de choix classe / série / matière** (maquette V2.1, **écran 3**) : le premier vrai écran de navigation du contenu. L'étape 13 (comptes élève) est faite ; l'étape 14 dépend de l'étape 11 (composants accessibles, faits). C'est cet écran qui **renseigne enfin `classe_id`/`serie`** du profil `utilisateur` (restés `null` depuis l'inscription), via la policy `utilisateur_maj_de_soi` de la migration 03, et qui **remplace l'accueil-galerie provisoire** (`galerie_composants_screen.dart`, avec son bouton de déconnexion temporaire). Ouvrir la maquette écran 3 + `SPECIFICATIONS_V2_Plateforme.md` **avant de coder**. Rappels de vocabulaire figé (`docs/GLOSSAIRE.md` §2) : la **série** (L/S1/S2) n'est demandée qu'en **1ère/Terminale**, et **avant** les matières ; en série L, pas de physique-chimie ; une matière n'apparaît que si sa bibliothèque existe réellement (sinon « bientôt disponible » — au lancement, seule la 6e Maths est complète).

> **Séquencement (Feuille de Route, doc 04).** **12 (faite) → 13 (faite) → 14 (écran choix classe/série/matière) → 15 (liste des chapitres, qui attend 12 ET 14)**. La prochaine étape est donc la **14**.
