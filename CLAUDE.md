...
## 6. État actuel du projet (dernière mise à jour : 11 août 2026)
...
**Étape 15 — Liste des chapitres** (maquette V2.1, **écran 4**) : le premier écran de contenu réel. Elle dépend de l'étape 12 (modèle de données, entité `Chapitre` faite) **ET** de l'étape 14 (choix classe/série/matière, faite) : l'élève, ayant choisi sa classe et sa matière, voit la liste ordonnée des chapitres de cette (classe, matière), regroupés par **strate** (« Activités Numériques »/« Activités Géométriques » en 6e Maths, `docs/GLOSSAIRE.md` §2). Ouvrir la maquette écran 4 + `SPECIFICATIONS_V2_Plateforme.md` **avant de coder**. Points d'attention connus : (a) **aucun chapitre n'existe encore en base** — le contenu réel (les 19 chapitres de 6e Maths) relève de l'étape 18, donc au lancement la liste sera **vide** : c'est le bon moment pour brancher le **« contenu indisponible » dynamique** (SPEC §7), volontairement **distinct** du badge statique `BibliothequesPretes` de l'étape 14 ; (b) offline-first via un `ChapitreRepository` (le comptage `matieresAvecBibliotheque` évoqué à l'étape 14 pourra y vivre) ; (c) l'entrée se fait depuis la classe/matière retenues — connecté via `profilProvider`, invité via `choixClasseProvider`.

> **Séquencement (Feuille de Route, doc 04).** **12 (faite) → 13 (faite) → 14 (faite) → 15 (liste des chapitres, attend 12 ET 14)**. La prochaine étape est donc la **15**.

Ensuite les écrans restants de la V2.1 un par un, en ouvrant à chaque fois la maquette (`FayeMath_Maquettes_Ecrans_V2.html`) et ses spécifications avant de coder.
