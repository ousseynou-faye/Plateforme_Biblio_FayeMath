...
## 6. État actuel du projet (dernière mise à jour : 13 août 2026)
...
**Étape 17 — Lecteur de documents** (maquette V2.1, **écran 7**) : depuis l'écran de détail (étape 16), l'ouverture d'un document affiche son contenu dans un **lecteur PDF** (décision §3 : `syncfusion_flutter_pdfviewer` **ou** `pdfx`, à trancher au démarrage de l'étape), avec défilement, pagination (« Page X sur N »), contrôle de la taille du texte, et une barre basse qui indique la **disponibilité locale** du document (maquette). Elle dépend de l'étape 16 : la ligne de document y attend déjà une navigation, aujourd'hui un placeholder SnackBar « L'ouverture du document arrive bientot ». Ouvrir la maquette **écran 7** + `SPECIFICATIONS_V2_Plateforme.md` **avant de coder**. Points d'attention connus : (a) **aucun document réel n'existe encore** (contenu = étape 18) et le **téléchargement réel** (dio + Storage) relève de la **Phase 3** — prévoir l'affichage quand rien n'est en local, et envisager le **seed de démonstration temporaire** évoqué à l'étape 16 pour vérifier le lecteur en vrai sur appareil ; (b) offline-first : lire d'abord le document **déjà sur l'appareil** ; (c) ne jamais confondre **état du réseau** (bandeau) et **disponibilité du document** (barre basse) — GLOSSAIRE §6.

> **Séquencement (Feuille de Route, doc 04).** **12 → 13 → 14 → 15 (faites) → 16 (livrée le 15/08 ; run appareil de non-régression à confirmer) → 17 (lecteur de documents, écran 7, attend 16)**. La prochaine étape est donc la **17**.

Ensuite les étapes restantes de la V2.1 un par un (18 mise en ligne du contenu, puis la Phase 3 hors-ligne…), en ouvrant à chaque fois la maquette (`FayeMath_Maquettes_Ecrans_V2.html`) et ses spécifications avant de coder.
