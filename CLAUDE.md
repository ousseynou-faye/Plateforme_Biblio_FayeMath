...
## 6. État actuel du projet (dernière mise à jour : 2 août 2026)
...
- **Étapes 10-11** : pas commencées, dépendent de l'étape 9. Outils Riverpod/go_router/Drift/dio (2 j) → charte graphique et composants accessibles (3,5 j).
- **Git** : ✅ dépôt initialisé, remote privé `origin` = `https://github.com/ousseynou-faye/Plateforme_Biblio_FayeMath.git`, branche `main`. 2 commits (`e69efec` projet Flutter, `599e67e` socle technique + assets). **Au 01/08 : 1 commit non poussé (`ahead 1`).**
...
**Étape 10 — Installer et câbler les outils clés** : ajouter au `pubspec.yaml` (zone 🟠 — annoncer chaque dépendance, une par une, avec son état de maintenance sur pub.dev, `SECURITY.md` §8) `flutter_riverpod`, `go_router`, `drift` (+ son générateur de code), `dio`, et le client `supabase_flutter` (décision reportée à cette étape, pas avant). Puis câbler le socle, sans écran métier : `ProviderScope` à la racine, un `go_router` minimal, la base locale Drift, et la lecture des secrets via `--dart-define-from-file=config/dev.json` (typiquement `lib/core/env/env.dart`, jamais de clé en dur). Respecter la règle de dépendance de `docs/ARCHITECTURE.md` (le socle réseau/local vit dans `data/`, pas dans un écran).

Puis étape 11 (charte graphique et composants accessibles). Ensuite seulement : construire les 20 écrans de la V1 un par un, en suivant la maquette V2.1.
