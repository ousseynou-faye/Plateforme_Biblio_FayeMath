...
- **Étape 11** : pas commencée. Charte graphique et composants accessibles (3,5 j).
- **Git** : ✅ dépôt initialisé, remote privé `origin` = `https://github.com/ousseynou-faye/Plateforme_Biblio_FayeMath.git`, branche `main`. Commits `e69efec` (projet Flutter), `599e67e` (socle + assets), plus les commits des étapes 8-9. **Le commit de l'étape 10 est en préparation** (relu avant `git commit`). Vérifier l'état `ahead`/push avec `git status` en début de session.
...
**Étape 11 — Charte graphique et composants accessibles** : construire dans `lib/core/theme/` le thème de l'app à partir de la charte (§5) — indigo `#16213E`, ocre `#B8622F`, sable `#F2E8DC`, doré `#C9A15A` — sous forme de `ThemeData` centralisé (aucune valeur brute hors du thème, `docs/CONVENTIONS.md` §4). **Choisir ensemble la police d'interface** (pistes Poppins/Questrial/Jost, ne pas embarquer Century Gothic). Poser les premiers composants réutilisables accessibles (`presentation/widgets/`, suffixe `_widget.dart`) en respectant les contraintes d'accessibilité de `02 - Contenu et Experience.pdf`. C'est là qu'on **remplace les écrans/table de démonstration de l'étape 10** (compteur, seconde page, `LignesDemo`, preuve Drift dans `main.dart`).

Ensuite seulement : construire les 20 écrans de la V2.1 un par un, en ouvrant à chaque fois la maquette (`FayeMath_Maquettes_Ecrans_V2.html`) et ses spécifications avant de coder.
